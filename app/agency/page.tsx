...
export default function AgencyDashboard(){
 ...
 const [loading,setLoading]=useState(true),[saving,setSaving]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(''),[search,setSearch]=useState('')
 const [createOpen,setCreateOpen]=useState(false)
 const [form,setForm]=useState({full_name:'',email:'',phone:'',password:'',product_id:''})
 ...
 useEffect(()=>{void load()},[])
 const availableProducts=useMemo(()=>products.filter(p=>p.status==='active'&&(!p.expires_at||new Date(p.expires_at).getTime()>Date.now())&&Number(p.remaining_slots)>0),[products])
 const openCreate=()=>{setError('');setNotice('');setForm({full_name:'',email:'',phone:'',password:'',product_id:availableProducts[0]?.product_id??''});setCreateOpen(true)}
 const createMember=async(e:FormEvent)=>{e.preventDefault();setSaving(true);setError('');setNotice('');try{const {data:{session}}=await supabase.auth.getSession();if(!session?.access_token)throw new Error('Session Agency tidak ditemukan. Silakan login ulang.');const res=await fetch('/api/agency/members',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify(form)});const body=await res.json().catch(()=>({}));if(!res.ok)throw new Error(body.error||'Gagal membuat Member.');setNotice('Member berhasil dibuat. Product Access aktif dan slot Agency sudah terpakai 1.');setCreateOpen(false);setForm({full_name:'',email:'',phone:'',password:'',product_id:''});await load()}catch(err:unknown){setError(err instanceof Error?err.message:'Gagal membuat Member.')}finally{setSaving(false)}}
 const revoke=async(g:Grant)=>{if(!confirm(`Cabut akses ${g.product_name} dari ${g.member_name||'Member'}?`))return;setSaving(true);setError('');setNotice('');const {error:x}=await supabase.rpc('agency_revoke_product_access',{p_member_user_id:g.member_user_id,p_product_id:g.product_id});if(x)setError(x.message);else{setNotice('Product Access berhasil dicabut. Slot kembali tersedia.');await load()}setSaving(false)}
 ...
 return <div className="agency-page">
  ...
  <main>
   ...
   <div className="content">
    <header><div><em>AGENCY CONTROL CENTER</em><h1>Selamat datang, {me?.full_name||'Agency'} 👋</h1><p>Kelola produk Agency, kuota lisensi, dan member Anda dari satu dashboard.</p></div><div className="actions"><button onClick={()=>void load()}>↻ Refresh</button><button className="primary" onClick={openCreate} disabled={!availableProducts.length}>＋ Buat Member</button></div></header>
    ...
    <section className="panel" id="members"><div className="panel-head"><div><em>MY MEMBERS</em><h2>Member Saya</h2><p>Hanya member yang berada di bawah Agency Anda.</p></div><button className="primary" onClick={openCreate} disabled={!availableProducts.length}>＋ Buat Member</button></div>
