...
export default function AdminResourcesPage() {
  ...
  const [form, setForm] = useState<ResourceForm>({ product_id:'', title:'', description:'', file_url:'', file_type:'download', sort_order:'0', is_published:true })
  ...
  const load = async () => {
    ...
    const [resourceResult, productResult] = await Promise.all([
      supabase.rpc('admin_list_product_resources'),
      supabase.from('products').select('id,name').order('name', { ascending:true }),
    ])
    const errors:string[] = []
    if (resourceResult.error) errors.push(`Resources: ${resourceResult.error.message}`)
    else setRows((resourceResult.data ?? []) as ResourceRow[])
    if (productResult.error) errors.push(`Products: ${productResult.error.message}`)
    else setProducts((productResult.data ?? []) as ProductOption[])
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) =>
      (!q || row.title.toLowerCase().includes(q) || row.product_name.toLowerCase().includes(q) || (row.description ?? '').toLowerCase().includes(q)) &&
      (statusFilter === 'all' || (statusFilter === 'published' ? row.is_published : !row.is_published)) &&
  ...
  const productCount = new Set(rows.map((r) => r.product_id)).size
  const types = new Set(rows.map((r) => r.file_type)).size

  const openCreate = () => {
    setSelected(null); setError(''); setNotice('')
    setForm({ product_id:products[0]?.id ?? '', title:'', description:'', file_url:'', file_type:'download', sort_order:'0', is_published:true })
    setModal(true)
  }

  const openEdit = (row:ResourceRow) => {
    setSelected(row); setError(''); setNotice('')
    setForm({ product_id:row.product_id, title:row.title, description:row.description ?? '', file_url:row.file_url, file_type:row.file_type, sort_order:String(row.sort_order ?? 0), is_published:row.is_published })
    setModal(true)
  }
  ...
  const save = async (event:FormEvent) => {
    ...
    const args = {
      ...
      p_file_url:form.file_url.trim(),
      p_file_type:form.file_type,
      p_sort_order:Number(form.sort_order || 0),
      p_is_published:form.is_published,
    }
    const result = selected
      ? await supabase.rpc('admin_update_product_resource',{ p_resource_id:selected.id, ...args })
      : await supabase.rpc('admin_create_product_resource',args)
    if (result.error) setError(result.error.message)
    else { setNotice(selected ? 'Resource berhasil diperbarui.' : 'Resource berhasil ditambahkan.'); setModal(false); await load() }
    setSaving(false)
  }

  const remove = async (row:ResourceRow) => {
    if (!confirm(`Hapus resource "${row.title}"?`)) return
    setSaving(true); setError(''); setNotice('')
    const { error:deleteError } = await supabase.rpc('admin_delete_product_resource',{ p_resource_id:row.id })
    if (deleteError) setError(deleteError.message)
    else { setNotice('Resource berhasil dihapus.'); await load() }
    setSaving(false)
  }
  ...
  return <div className="admin-page">
    ...
    <main>
      <div className="topbar"><div className="search-top">⌕ <span>Cari member, produk atau materi...</span></div><div className="top-actions"><button className="mini-profile"><span className="avatar small">{initials}</span><span><strong>{me?.full_name || email.split('@')[0]}</strong><small>{me?.role === 'super_admin' ? 'Super Admin' : 'Admin'}</small></span></button><ThemeSwitcher /></div></div>
      <div className="content">
        <header><div><div className="eyebrow">DIGITAL ASSETS</div><h1>Resources</h1><p>Kelola file download, bonus, template, link, dan resource untuk setiap produk.</p></div><div style={{display:'flex',gap:8}}><button className="refresh" onClick={() => void load()}>↻ Refresh</button><button className="primary" onClick={openCreate}>＋ Tambah Resource</button></div></header>
        {error && <div className="alert error">{error}</div>}{notice && <div className="alert success">{notice}</div>}

        <section className="stats">
          <article className="stat blue"><div className="stat-icon">◆</div><div><span>TOTAL RESOURCES</span><strong>{rows.length}</strong><small>Semua asset digital</small></div></article>
          <article className="stat purple"><div className="stat-icon">✓</div><div><span>PUBLISHED</span><strong>{published}</strong><small>Terlihat untuk member</small></div></article>
          <article className="stat green"><div className="stat-icon">▣</div><div><span>PRODUCTS</span><strong>{productCount}</strong><small>Produk memiliki resource</small></div></article>
          <article className="stat pink"><div className="stat-icon">⌘</div><div><span>RESOURCE TYPES</span><strong>{types}</strong><small>{drafts} draft / hidden</small></div></article>
        </section>

        <section className="panel">
          <div className="panel-head"><div><div className="eyebrow">RESOURCE LIBRARY</div><h2>Product Resources</h2><p>Gunakan URL HTTPS untuk file atau resource eksternal. Data tetap terhubung ke produk.</p></div><span className="summary-chip">{filtered.length} resource ditemukan</span></div>
          <div className="filters">
            <div className="searchbox">⌕<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari resource atau produk..." /></div>
            <select value={productFilter} onChange={(e) => setProductFilter(e.target.value)}><option value="all">Semua Produk</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="all">Semua Status</option><option value="published">Published</option><option value="draft">Draft</option></select>
          ...
          <div className="table-wrap"><table><thead><tr><th>RESOURCE</th><th>PRODUCT</th><th>TYPE</th><th>STATUS</th><th>ORDER</th><th>UPDATED</th><th>ACTION</th></tr></thead><tbody>
            {loading ? <tr><td colSpan={7} className="empty">Loading resources...</td></tr> : visible.length === 0 ? <tr><td colSpan={7} className="empty">Belum ada resource yang sesuai filter.</td></tr> : visible.map((row) => <tr key={row.id}>
              <td><div className="member-cell"><span className="member-avatar">◆</span><div><strong>{row.title}</strong><small>{row.description || row.file_url}</small></div></div></td>
              <td><strong>{row.product_name}</strong></td><td>{pretty(row.file_type)}</td><td><span className={`badge ${row.is_published ? 'active' : 'inactive'}`}>{row.is_published ? 'Published' : 'Draft'}</span></td><td>{row.sort_order}</td><td>{date(row.updated_at)}</td>
              <td><div style={{display:'flex',gap:6}}><button className="action" onClick={() => window.open(row.file_url,'_blank','noopener,noreferrer')}>Buka</button><button className="action" onClick={() => openEdit(row)}>Edit</button><button className="action" onClick={() => void remove(row)}>Hapus</button></div></td>
            </tr>)}
          </tbody></table></div>
    ...
    {modal && <div className="modal-backdrop" onMouseDown={() => !saving && setModal(false)}><div className="modal" onMouseDown={(e) => e.stopPropagation()}>
      <div className="modal-head"><div><div className="eyebrow">{selected ? 'EDIT RESOURCE' : 'NEW RESOURCE'}</div><h2>{selected ? 'Edit Resource' : 'Tambah Resource'}</h2><p>Resource akan mengikuti akses produk member.</p></div><button className="close" onClick={() => setModal(false)}>×</button></div>
      <form onSubmit={save}>
        <label>Produk<select value={form.product_id} onChange={(e) => setForm({...form,product_id:e.target.value})} required><option value="">Pilih Produk</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
        <label>Judul Resource<input value={form.title} onChange={(e) => setForm({...form,title:e.target.value})} placeholder="Contoh: Template Canva Bonus" required /></label>
        <label>Deskripsi<input value={form.description} onChange={(e) => setForm({...form,description:e.target.value})} placeholder="Deskripsi singkat..." /></label>
        <label>URL Resource<input type="url" value={form.file_url} onChange={(e) => setForm({...form,file_url:e.target.value})} placeholder="https://..." required /></label>
        <label>Tipe<select value={form.file_type} onChange={(e) => setForm({...form,file_type:e.target.value})}><option value="download">Download / File</option><option value="template">Template</option><option value="bonus">Bonus</option><option value="external_url">External Link</option><option value="document">Document</option><option value="other">Other</option></select></label>
        <label>Urutan<input type="number" min="0" value={form.sort_order} onChange={(e) => setForm({...form,sort_order:e.target.value})} /></label>
        <label style={{display:'flex',alignItems:'center',gap:9,flexDirection:'row'}}><input type="checkbox" checked={form.is_published} onChange={(e) => setForm({...form,is_published:e.target.checked})} style={{width:16,height:16}} /> Published — tampil untuk member</label>
        <div className="modal-actions"><button type="button" className="secondary" onClick={() => setModal(false)}>Batal</button><button className="primary" disabled={saving || !form.product_id || !form.title.trim() || !form.file_url.trim()}>{saving ? 'Menyimpan...' : selected ? 'Simpan Perubahan' : 'Tambah Resource'}</button></div>
      </form>
    </div></div>}
      <style jsx>{`
