...
export default function AdminNotificationsPage(){
 ...
 const [tab,setTab]=useState<'outbox'|'templates'>('outbox'),[rows,setRows]=useState<Outbox[]>([]),[templates,setTemplates]=useState<Template[]>([])
 ...
 },[tab,page,pageSize,search,channel,status])

 useEffect(()=>{void (async()=>{if(await loadIdentity())await loadData()})()},[loadIdentity,loadData])
 useEffect(()=>{if(page>pages)setPage(pages)},[page,pages])

 const stats=useMemo(()=>({queued:rows.filter(x=>x.status==='queued').length,sent:rows.filter(x=>x.status==='sent').length,failed:rows.filter(x=>x.status==='failed').length}),[rows])
...
<main className="content">
 <div className="nt-head"><div><div className="nt-kicker">COMMERCE / NOTIFICATIONS</div><h1>Notification Management</h1><p>Kelola antrean notifikasi, status pengiriman, dan template komunikasi platform.</p></div><div className="nt-actions"><button className="nt-btn" onClick={()=>void loadData()} disabled={loading}>↻ Refresh</button></div></div>
 {error&&<div className="nt-alert error">{error}</div>}{message&&<div className="nt-alert ok">{message}</div>}
 <section className="nt-stats">
  <div className="nt-stat blue"><span>◎</span><div><small>DATA HALAMAN INI</small><strong>{tab==='outbox'?rows.length:templates.length}</strong><p>Sesuai filter aktif</p></div></div>
  <div className="nt-stat purple"><span>⌛</span><div><small>QUEUED</small><strong>{stats.queued}</strong><p>Menunggu worker</p></div></div>
  <div className="nt-stat mint"><span>✓</span><div><small>SENT</small><strong>{stats.sent}</strong><p>Berhasil dikirim</p></div></div>
  <div className="nt-stat pink"><span>!</span><div><small>FAILED</small><strong>{stats.failed}</strong><p>Dapat diretry admin</p></div></div>
 </section>
 ...
 <section className="nt-card">
  ...
  {tab==='templates'&&<div className="nt-variables"><div className="nt-variables-head"><div><strong>Variable / Parameter Template</strong><span>Klik variable untuk menyalin. Tempelkan ke Subject Email atau Isi Pesan.</span></div></div><div className="nt-variable-list">{['{{name}}','{{order_number}}','{{grand_total}}','{{currency}}','{{product_name}}','{{payment_method}}','{{payment_status}}','{{order_status}}','{{buyer_name}}','{{buyer_email}}','{{buyer_phone}}','{{amount}}','{{discount_amount}}','{{payment_fee}}','{{support_email}}','{{support_whatsapp}}'].map(v=><button key={v} type="button" className="nt-variable" title={`Copy ${v}`} onClick={()=>void navigator.clipboard?.writeText(v)}>{v}</button>)}</div><p className="nt-variable-note">Gunakan hanya variable yang tersedia pada event/trigger terkait. Contoh: <b>{'Pesanan {{order_number}} sebesar {{grand_total}} {{currency}} berhasil dibayar.'}</b></p></div>}
...
.nt-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}.nt-head h1{font-size:26px;line-height:1.15;margin:6px 0;color:#111827}.nt-head p{margin:0;color:#778198;font-size:13px}.nt-kicker{font-size:10px;font-weight:900;letter-spacing:1.4px;color:#6657ef}.nt-actions{display:flex;gap:8px}.nt-btn{height:42px;padding:0 15px;border:1px solid #e1e5ef;border-radius:11px;background:#fff;font-size:11px;font-weight:850;cursor:pointer}.nt-btn.primary{border:0;color:#fff;background:linear-gradient(135deg,#6657ef,#8059ff)}.nt-btn:disabled{opacity:.45}
.nt-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:13px;margin-bottom:15px}.nt-stat{min-height:108px;border:1px solid rgba(124,92,255,.12);border-radius:16px;padding:16px;display:flex;gap:13px;box-shadow:0 14px 34px rgba(72,77,120,.05)}.nt-stat.blue{background:linear-gradient(135deg,#eef5ff,#f8fbff 52%,#f1efff)}.nt-stat.purple{background:linear-gradient(135deg,#f5efff,#fbf8ff 55%,#eeeaff)}.nt-stat.mint{background:linear-gradient(135deg,#eafff5,#f5fffb 52%,#edf5ff)}.nt-stat.pink{background:linear-gradient(135deg,#fff0f8,#fff8fc 52%,#f4efff)}.nt-stat>span{width:38px;height:38px;border-radius:12px;background:rgba(255,255,255,.68);display:grid;place-items:center;color:#6758ea;font-weight:900}.nt-stat small{font-size:9px;color:#7d869d}.nt-stat strong{display:block;font-size:19px;margin:4px 0}.nt-stat p{font-size:10px;color:#9aa2b4;margin:0}
.nt-card{background:linear-gradient(145deg,#fff,#fbfbff 52%,#f7f3ff);border:1px solid rgba(124,92,255,.12);border-radius:18px;overflow:hidden;box-shadow:0 16px 40px rgba(72,77,120,.05)}.nt-card-head{padding:18px 20px 14px;border-bottom:1px solid rgba(124,92,255,.08)}.nt-card-head h2{font-size:16px;margin:4px 0 3px}.nt-card-head p{font-size:11px;color:#8a93a8;margin:0}.nt-tabs{display:flex;gap:7px;padding:12px 20px 0}.nt-tab{height:35px;padding:0 12px;border:1px solid #e2e5ef;border-radius:9px;background:#fff;font-size:10px;font-weight:800;cursor:pointer}.nt-tab.active{border-color:#cfc7ff;background:#f0edff;color:#5e4ae1}
.nt-variables{margin:12px 20px 0;padding:13px 14px;border:1px solid rgba(124,92,255,.14);border-radius:13px;background:linear-gradient(135deg,#f3f0ff,#f8fbff 55%,#effbf7)}.nt-variables-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}.nt-variables-head>div{display:grid;gap:3px}.nt-variables-head strong{font-size:11px;color:#3f4658}.nt-variables-head span{font-size:9px;color:#8b94a7}.nt-variable-list{display:flex;flex-wrap:wrap;gap:6px}.nt-variable{border:1px solid #dcd7ff;border-radius:8px;padding:6px 8px;background:rgba(255,255,255,.82);color:#5f50d7;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-size:10px;font-weight:800;cursor:pointer}.nt-variable:hover{background:#ebe7ff;border-color:#bdb3ff}.nt-variable-note{margin:9px 0 0;color:#7f899e;font-size:9px;line-height:1.5}.nt-variable-note b{color:#59647a;font-weight:700}
