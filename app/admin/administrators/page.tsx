...
export default function AdminAdministratorsPage() {
  ...
  const [candidates,setCandidates] = useState<AdministratorRow[]>([])
  ...
  const [selected,setSelected] = useState<AdministratorRow|null>(null)
  const [form,setForm] = useState<AdminForm>({user_id:'',full_name:'',phone:'',role:'admin',status:'active'})
  ...
  const load = async () => {
    ...
    const [adminsResult,candidatesResult]=await Promise.all([
      supabase.rpc('super_admin_list_administrators'),
      supabase.rpc('super_admin_list_admin_candidates'),
    ])
    const errors:string[]=[]
    if(adminsResult.error) errors.push(`Administrators: ${adminsResult.error.message}`)
    else setRows((adminsResult.data??[]) as AdministratorRow[])
    if(candidatesResult.error) errors.push(`Candidates: ${candidatesResult.error.message}`)
    else setCandidates((candidatesResult.data??[]) as AdministratorRow[])
  ...
  const filtered=useMemo(()=>{
    ...
    return rows.filter(row=>
      (!q||(row.full_name??'').toLowerCase().includes(q)||(row.phone??'').toLowerCase().includes(q)||row.id.toLowerCase().includes(q)) &&
  ...
  const superCount=rows.filter(r=>r.role==='super_admin'&&r.status==='active').length
  const adminCount=rows.filter(r=>r.role==='admin'&&r.status==='active').length
  const inactiveCount=rows.filter(r=>r.status!=='active').length

  const openAdd=()=>{
    setSelected(null);setError('');setNotice('')
    const first=candidates[0]
    setForm({user_id:first?.id??'',full_name:first?.full_name??'',phone:first?.phone??'',role:'admin',status:'active'})
    setModal(true)
  }
  const openEdit=(row:AdministratorRow)=>{
    setSelected(row);setError('');setNotice('')
    setForm({user_id:row.id,full_name:row.full_name??'',phone:row.phone??'',role:row.role??'admin',status:row.status??'active'})
    setModal(true)
  }
  const chooseCandidate=(id:string)=>{
    const c=candidates.find(x=>x.id===id)
    setForm(f=>({...f,user_id:id,full_name:c?.full_name??'',phone:c?.phone??''}))
  }
  const save=async(e:FormEvent)=>{
    e.preventDefault();setSaving(true);setError('');setNotice('')
    const {error:saveError}=await supabase.rpc('super_admin_upsert_administrator',{
    ...
    else{setNotice(selected?'Administrator berhasil diperbarui.':'Akun berhasil dijadikan administrator.');setModal(false);await load()}
    setSaving(false)
  }
  const deactivate=async(row:AdministratorRow)=>{
    if(row.id===me?.id){setError('Akun Super Admin yang sedang digunakan tidak dapat dinonaktifkan.');return}
    if(!confirm(`Nonaktifkan ${row.full_name||'administrator'}?`))return
    setSaving(true);setError('');setNotice('')
    const {error:x}=await supabase.rpc('super_admin_deactivate_administrator',{p_user_id:row.id})
    if(x)setError(x.message);else{setNotice('Administrator dinonaktifkan.');await load()}
    setSaving(false)
  }
  ...
  return <div className="admin-page">
    ...
    <main>
      <div className="topbar"><div className="search-top">⌕ <span>Cari administrator...</span></div><div className="top-actions"><button className="mini-profile"><span className="avatar small">{initials}</span><span><strong>{me?.full_name||email.split('@')[0]}</strong><small>Super Admin</small></span></button><ThemeSwitcher/></div></div>
      <div className="content">
        <header><div><div className="eyebrow">SUPER ADMIN CONTROL</div><h1>Administrators</h1><p>Kelola akun yang diberi akses ke Admin Control iMersSUPA.</p></div><div style={{display:'flex',gap:8}}><button className="refresh" onClick={()=>void load()}>↻ Refresh</button><button className="primary" onClick={openAdd} disabled={!candidates.length}>＋ Tambah Administrator</button></div></header>
        {error&&<div className="alert error">{error}</div>}{notice&&<div className="alert success">{notice}</div>}
        <section className="stats">
          <article className="stat blue"><div className="stat-icon">♛</div><div><span>TOTAL ADMIN</span><strong>{rows.length}</strong><small>Semua akun administrator</small></div></article>
          <article className="stat purple"><div className="stat-icon">★</div><div><span>SUPER ADMIN</span><strong>{superCount}</strong><small>Super Admin aktif</small></div></article>
          <article className="stat green"><div className="stat-icon">✓</div><div><span>ADMIN ACTIVE</span><strong>{adminCount}</strong><small>Administrator aktif</small></div></article>
          <article className="stat pink"><div className="stat-icon">○</div><div><span>INACTIVE</span><strong>{inactiveCount}</strong><small>Akun admin nonaktif</small></div></article>
        </section>

        <section className="panel">
          <div className="panel-head"><div><div className="eyebrow">ADMIN DIRECTORY</div><h2>Administrator Accounts</h2><p>Untuk keamanan, administrator baru dipilih dari akun iMersSUPA yang sudah terdaftar.</p></div><span className="summary-chip">{filtered.length} administrator</span></div>
          <div className="filters"><div className="searchbox">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cari nama, telepon atau User ID..."/></div><select value={roleFilter} onChange={e=>setRoleFilter(e.target.value)}><option value="all">Semua Role</option><option value="super_admin">Super Admin</option><option value="admin">Admin</option></select><select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="all">Semua Status</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="suspended">Suspended</option></select><select value={pageSize} onChange={e=>setPageSize(Number(e.target.value))}>{PAGE_SIZES.map(n=><option key={n} value={n}>{n} / halaman</option>)}</select></div>
          <div className="table-wrap"><table><thead><tr><th>ADMINISTRATOR</th><th>ROLE</th><th>STATUS</th><th>PHONE</th><th>CREATED</th><th>ACTION</th></tr></thead><tbody>
            {loading?<tr><td colSpan={6} className="empty">Loading administrators...</td></tr>:visible.length===0?<tr><td colSpan={6} className="empty">Belum ada administrator yang sesuai filter.</td></tr>:visible.map(row=><tr key={row.id}><td><div className="member-cell"><span className="member-avatar">{(row.full_name||'A').slice(0,1).toUpperCase()}</span><div><strong>{row.full_name||'Administrator'}</strong><small>{row.id===me?.id?'Akun Anda':row.id.slice(0,12)+'...'}</small></div></div></td><td><span className={`badge ${row.role==='super_admin'?'suspended':'active'}`}>{pretty(row.role||'admin')}</span></td><td><span className={`badge ${row.status==='active'?'active':'inactive'}`}>{pretty(row.status||'inactive')}</span></td><td>{row.phone||'—'}</td><td>{date(row.created_at)}</td><td><div style={{display:'flex',gap:6}}><button className="action" onClick={()=>openEdit(row)}>Edit</button>{row.id!==me?.id&&row.status==='active'&&<button className="action" onClick={()=>void deactivate(row)}>Nonaktifkan</button>}</div></td></tr>)}
          </tbody></table></div>
    ...
    {modal&&<div className="modal-backdrop" onMouseDown={()=>!saving&&setModal(false)}><div className="modal" onMouseDown={e=>e.stopPropagation()}>
      <div className="modal-head"><div><div className="eyebrow">{selected?'EDIT ADMINISTRATOR':'ADD ADMINISTRATOR'}</div><h2>{selected?'Edit Administrator':'Tambah Administrator'}</h2><p>{selected?'Atur role, status, dan profil administrator.':'Pilih akun member yang sudah terdaftar lalu berikan role Admin.'}</p></div><button className="close" onClick={()=>setModal(false)}>×</button></div>
      <form onSubmit={save}>
        {!selected&&<label>Akun Terdaftar<select value={form.user_id} onChange={e=>chooseCandidate(e.target.value)} required><option value="">Pilih akun</option>{candidates.map(c=><option key={c.id} value={c.id}>{c.full_name||c.id.slice(0,8)} {c.phone?`• ${c.phone}`:''}</option>)}</select></label>}
        <label>Nama<input value={form.full_name} onChange={e=>setForm({...form,full_name:e.target.value})} placeholder="Nama administrator"/></label>
        <label>Nomor WhatsApp / Telepon<input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="08..."/></label>
        <label>Role<select value={form.role} onChange={e=>setForm({...form,role:e.target.value})} disabled={selected?.id===me?.id}><option value="admin">Admin</option><option value="super_admin">Super Admin</option></select></label>
        <label>Status<select value={form.status} onChange={e=>setForm({...form,status:e.target.value})} disabled={selected?.id===me?.id}><option value="active">Active</option><option value="inactive">Inactive</option><option value="suspended">Suspended</option></select></label>
        <div className="modal-actions"><button type="button" className="secondary" onClick={()=>setModal(false)}>Batal</button><button className="primary" disabled={saving||!form.user_id}>{saving?'Menyimpan...':selected?'Simpan Perubahan':'Jadikan Administrator'}</button></div>
      </form>
    </div></div>}
      <style jsx>{`
