...
type ProgressRow = {
  ...
  last_position: string | number | null
...
export default function AdminProgressPage() {
  ...
  const load = async () => {
    ...
    const [progressResult, productResult] = await Promise.all([
      supabase.rpc('admin_list_member_progress'),
      supabase.from('products').select('id,name').order('name', { ascending: true }),
    ])

    if (progressResult.error) setError(`Progress: ${progressResult.error.message}`)
    else setRows((progressResult.data ?? []) as ProgressRow[])

    if (productResult.error) {
      setError((old) => [old, `Products: ${productResult.error?.message}`].filter(Boolean).join(' • '))
    } else {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const status = row.completed ? 'completed' : percent > 0 ? 'in_progress' : 'not_started'
      const matchesSearch =
        !q ||
        row.member_name.toLowerCase().includes(q) ||
        row.product_name.toLowerCase().includes(q) ||
        row.content_title.toLowerCase().includes(q)
  ...
  const inProgress = rows.filter((r) => !r.completed && Number(r.progress_percent ?? 0) > 0).length
  const activeMembers = new Set(rows.filter((r) => Number(r.progress_percent ?? 0) > 0 || r.completed).map((r) => r.user_id)).size
  const avg = rows.length ? Math.round(rows.reduce((sum, r) => sum + Number(r.progress_percent ?? 0), 0) / rows.length) : 0
  ...
  return <div className="admin-page">
    <aside className="sidebar">
      ...
      <nav>
        ...
        <button className="active">↗ <span>Progress</span></button>
      ...
      <div className="sidebar-bottom">
        <button className="profile-card" onClick={() => router.push('/admin/profile')}>
          ...
          <span className="profile-copy"><strong>{me?.full_name || email.split('@')[0]}</strong><small>{email}</small></span>
        </button>
    ...
    <main>
      <div className="topbar">
        ...
        <div className="top-actions">
          <button className="mini-profile"><span className="avatar small">{initials}</span><span><strong>{me?.full_name || email.split('@')[0]}</strong><small>{me?.role === 'super_admin' ? 'Super Admin' : 'Admin'}</small></span></button>
          <ThemeSwitcher />
        </div>
      </div>
      ...
      <div className="content">
        <header>
          <div><div className="eyebrow">LEARNING ANALYTICS</div><h1>Progress</h1><p>Pantau aktivitas belajar dan penyelesaian materi seluruh member.</p></div>
        ...
        <section className="stats">
          <article className="stat blue"><div className="stat-icon">◎</div><div><span>ACTIVE LEARNERS</span><strong>{activeMembers}</strong><small>Member mulai belajar</small></div></article>
          <article className="stat purple"><div className="stat-icon">↗</div><div><span>IN PROGRESS</span><strong>{inProgress}</strong><small>Materi sedang berjalan</small></div></article>
          <article className="stat green"><div className="stat-icon">✓</div><div><span>COMPLETED</span><strong>{completed}</strong><small>Materi diselesaikan</small></div></article>
          <article className="stat pink"><div className="stat-icon">%</div><div><span>AVG PROGRESS</span><strong>{avg}%</strong><small>Rata-rata semua record</small></div></article>
        </section>

        <section className="panel">
          <div className="panel-head">
            <div><div className="eyebrow">PROGRESS DIRECTORY</div><h2>Member Learning Progress</h2><p>Data dibaca dari progress asli Member Area, bukan data dummy.</p></div>
            <span className="summary-chip">{filtered.length} progress ditemukan</span>
          </div>

          <div className="filters">
            <div className="searchbox">⌕<input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari member, produk, atau materi..." /></div>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>MEMBER</th><th>PRODUCT</th><th>MATERI</th><th>PROGRESS</th><th>STATUS</th><th>LAST ACTIVITY</th></tr></thead>
              <tbody>
                {loading ? <tr><td colSpan={6} className="empty">Loading progress...</td></tr> :
                visible.length === 0 ? <tr><td colSpan={6} className="empty">Belum ada progress yang sesuai filter.</td></tr> :
                visible.map((row) => {
                  const percent = Math.max(0, Math.min(100, Number(row.progress_percent ?? 0)))
                  const status = row.completed ? 'completed' : percent > 0 ? 'in_progress' : 'not_started'
                  return <tr key={row.id}>
                    <td><div className="member-cell"><span className="member-avatar">{row.member_name.slice(0,1).toUpperCase()}</span><div><strong>{row.member_name}</strong><small>{row.member_phone || row.user_id.slice(0,12) + '...'}</small></div></div></td>
                    <td><strong>{row.product_name}</strong></td>
                    <td><div className="access-list"><strong>{row.content_title}</strong><small>{row.section_title || pretty(row.content_type)}</small></div></td>
                    <td><div style={{minWidth:120}}><strong>{percent}%</strong><div style={{height:6,background:'#edf0f6',borderRadius:999,marginTop:6,overflow:'hidden'}}><div style={{height:'100%',width:`${percent}%`,background:'linear-gradient(90deg,#6366f1,#8b5cf6)',borderRadius:999}} /></div></div></td>
                    <td><span className={`badge ${status === 'completed' ? 'active' : status === 'in_progress' ? 'suspended' : 'inactive'}`}>{pretty(status)}</span></td>
